...
import type { DeputadoResumo, FornecedorResumo, DespesaDetalhada, DeputadoDetalhado } from '@a-republica/shared';
// CacheService e constantes não são mais necessários aqui após unificação via loaders
// import { CacheService } from '@/services/cache-service';
// import { CACHE_KEYS, CACHE_PATHS } from '@/constants/cache';
import { loadSuppliersCache, loadDeputiesCache, loadAnalysisCache, loadPremiacoesCache } from './_cached-loaders'
import { z } from 'zod'
import { parseSuppliersCache, parseDeputiesCache, parseAnalysisCache, parsePremiacoesCache, parseDadosCompletosDeputado } from './analytics'

// ======================================================================================
// SECTION: Tipos de Dados Unificados
// Descrição: Estes são os tipos de dados "canônicos" que usaremos em toda a aplicação.
// A lógica de normalização abaixo garante que sempre trabalharemos com esta estrutura.
// ======================================================================================
...
export async function getDeputadoById(id: string): Promise<DeputadoResumo | undefined> {
  ...
  if (rawDeputados.length === 0) return undefined

  const rawDeputado = rawDeputados.find((d) => String(d.id) === id)
...
export async function getPremiacoes(filters: { ano?: string; categoria?: string; uf?: string } = {}) {
  ...
  const { getRankingGeral, getRankingPorAno, getRankingPorCategoria, getRankingFiltrado, calcularEstatisticasRanking } = await import('../../../services/unified-ranking-service');
  const { processarPremiacoes, identificarCampeaoGeral, processarCoroas, processarTrofeus, processarMedalhas } = await import('../../../services/premiacao-unificada');
...
export async function getDashboardData(filters: {
  ...
  const { fornecedores: allFornecedores, total: totalFornecedores } = await getFornecedores(); // Reutiliza a função já refatorada

  if (!rawDeputadosData || !rawDeputadosData.deputados) {
    // Retorna estrutura de dados vazia se não houver dados de deputados
    return {
      aggregates: { totalGasto: 0, totalDeputados: 0, totalFornecedores: 0, mediaGastoPorDeputado: 0 },
      topDeputados: [],
      topFornecedores: [],
      gastosPorPartido: [],
      gastosPorUF: [],
  ...
  const topDeputados = deputadosFiltrados
    .map(d => ({
      ...
      partido: d.siglaPartido,
      uf: d.siglaUf,
      totalDespesas: getGastoTotal(d, anoNum),
    }))
    .sort((a, b) => b.totalDespesas - a.totalDespesas)
    .slice((page - 1) * pageSize, page * pageSize);

  const topFornecedores = allFornecedores
    .map(f => ({
      cnpj: f.cnpjCpf,
      nome: f.nome,
      totalRecebido: anoNum
  ...
  const gastosPorPartidoMap = new Map<string, number>();
  deputadosFiltrados.forEach(d => {
    const p = d.siglaPartido;
    const gasto = getGastoTotal(d, anoNum);
    gastosPorPartidoMap.set(p, (gastosPorPartidoMap.get(p) || 0) + gasto);
  });

  const gastosPorPartido = Array.from(gastosPorPartidoMap.entries())
    .map(([partido, total]) => ({ partido, total }))
    .sort((a, b) => b.total - a.total)
    .slice((partidosPage - 1) * 10, partidosPage * 10);

  const gastosPorUFMap = new Map<string, number>();
  deputadosFiltrados.forEach(d => {
    const u = d.siglaUf;
    const gasto = getGastoTotal(d, anoNum);
    gastosPorUFMap.set(u, (gastosPorUFMap.get(u) || 0) + gasto);
  });
  ...
  const evolucaoTemporal = Array.from(anosSet)
    .sort((a, b) => a - b)
    .map(a => ({
      ano: a,
