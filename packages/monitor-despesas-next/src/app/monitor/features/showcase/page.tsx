...
import { DeputiesRanking, SuppliersRanking } from '@/features/ranking/RankingComponents'
import { ExibicaoPremiacoes, PremiacoesFilters, PremiacoesRankingList, PremiacoesStats } from '@/features/premiacoes/PremiacoesComponents'
import { GlobalSearchModal } from '@/features/search/GlobalSearchModal'
import ModernDeputadoSelector from '@/features/comparar/components/ModernDeputadoSelector'
import ComparisonSummary from '@/features/comparar/components/ComparisonSummary'
import ComparisonChart from '@/features/comparar/components/ComparisonChart'
import { GastosPorDeputado } from '@/features/ranking/RankingCharts'
import {
  Select,
  SelectContent,
  SelectItem,
...
export default function ShowcasePage() {
  return (
    <FilterProvider>
      <DashboardComponentsPageClient />
    </FilterProvider>
  )
}

function DashboardComponentsPageClient() {
  ...
  // Mock data for CardParlamentar
  const mockParlamentar = {
    id: '204554',
    nomeEleitoral: 'Dep. Exemplo',
    siglaPartido: 'XYZ',
    siglaUf: 'BR',
    totalDespesas: 123456.78,
    numeroDespesas: 99,
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-blue-50/30">
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <Tabs defaultValue="ui" className="space-y-12">
          <TabsList className="grid w-full grid-cols-6 h-auto p-2 bg-white/80 backdrop-blur-sm shadow-apple-md rounded-2xl border-0">
            ...
            <TabsTrigger value="features" className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-gray-700 data-[state=active]:to-blue-700 data-[state=active]:text-white rounded-xl py-3 font-medium transition-all">Features</TabsTrigger>
          </TabsList>

          {/* Features Tab: Custom Feature Components */}
          <TabsContent value="features" className="space-y-12">
            <section className="space-y-6 animate-slide-up">
              <h2 className="text-3xl font-bold text-gray-900">Componentes de Features</h2>

              {/* Monitor */}
              <Card className="mb-6"><CardHeader><CardTitle>Monitor</CardTitle></CardHeader><CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <DistribuicaoGastosPorCategoria data={[{ categoria: 'COMBUSTÍVEIS', valor: 15000, percentual: '60%' }, { categoria: 'PASSAGENS', valor: 10000, percentual: '40%' }]} />
                  <Top5Fornecedores items={[{ nome: 'Fornecedor Exemplo A', valor: 1234.56, numeroTransacoes: 10 }, { nome: 'Fornecedor Exemplo B', valor: 789.01, numeroTransacoes: 5 }]} />
                  <MiniCardStats items={[{ title: 'Total', value: 10000 }, { title: 'Média', value: 5000 }]} />

                  <MonitorNavigation />
                  <CardParlamentar deputado={mockParlamentar} />
                  <ExibicaoPremiacoes premiacoes={{ coroas: [], trofeus: [], medalhas: [], coroasOuro: [], trofeusPrata: [], medalhasBronze: [], badgesEspeciais: [], estatisticas: { totalPremiacoes: 0, totalCoroas: 0, totalTrofeus: 0, totalMedalhas: 0, totalBadgesEspeciais: 0 } }} />
                  <PremiacoesFilters anosDisponiveis={[2023]} categoriasDisponiveis={["DIVULGAÇÃO"]} ufsDisponiveis={["SP"]} anoSelecionado="2023" categoriaSelecionada="DIVULGAÇÃO" ufSelecionada="SP" onFilterChange={() => { }} onLimparFiltros={() => { }} isPending={false} showFilters={true} onToggleFilters={() => { }} />
                  <PremiacoesRankingList
                    ...
                    anoSelecionado="2023"
                  />
                  <PremiacoesStats estatisticas={{ totalDeputados: 0, campeao: 'N/A', maiorGasto: 0 }} />
                </div>
              </CardContent></Card>
              {/* Ranking */}
              <Card className="mb-6"><CardHeader><CardTitle>Ranking</CardTitle></CardHeader><CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <DeputiesRanking isPending={false} deputados={[]} pagination={{ page: 1, pageSize: 10, totalItems: 0, totalPages: 0 }} mediaPorDeputado={0} onPageChange={() => { }} />
                  <SuppliersRanking isPending={false} topFornecedores={{ items: [], pagination: { page: 1, pageSize: 10, totalItems: 0, totalPages: 0 } }} />
                </div>
              </CardContent></Card>
              {/* Busca */}
              <Card className="mb-6"><CardHeader><CardTitle>Busca</CardTitle></CardHeader><CardContent>
                <GlobalSearchModal />
              </CardContent></Card>

              {/* Comparar */}
              <Card className="mb-6"><CardHeader><CardTitle>Comparar</CardTitle></CardHeader><CardContent>
                <div className="space-y-4">
                  <ModernDeputadoSelector todosDeputados={[]} selectedIds={[]} onSelect={() => { }} onRemove={() => { }} />
                  <ComparisonSummary deputados={[]} />
                  <ComparisonChart deputados={[]} />
                </div>
              </CardContent></Card>
              {/* Dashboards */}
              <Card className="mb-6"><CardHeader><CardTitle>Dashboards</CardTitle></CardHeader><CardContent>
                <GastosPorDeputado deputados={[{ nome: 'Deputado X', valorTotal: 10000 }]} />
              </CardContent></Card>
              {/* Deputados */}
