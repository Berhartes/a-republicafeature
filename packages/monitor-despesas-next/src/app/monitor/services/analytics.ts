...
// Helper to accumulate yearly data from multiple sources
function accumulateYearlyData(sources: Array<Array<{ ano: number; total: number }>>) {
  const accumulator = new Map<number, number>()

  const append = (ano?: number, total?: number) => {
    if (!Number.isFinite(ano)) return
    const safeAno = Number(ano)
    const safeTotal = Number(total || 0)
    accumulator.set(safeAno, (accumulator.get(safeAno) || 0) + safeTotal)
  }

  sources.forEach(source => {
    source.forEach(entry => append(entry.ano, entry.total))
  })
...
const evolucaoAnualValueSchema = z.object({
  valor: z.number().optional(),
  transacoes: z.number().optional(),
  deputados: z.number().optional(),
})
...
const fornecedorRecordSchema = z.object({
  ...
  evolucaoAnual: z.record(z.string(), evolucaoAnualValueSchema).optional(),
}).passthrough()
...
export function normalizeFornecedor(record: SupplierRecord): NormalizedFornecedor {
  ...
  const anosFromEvolucao = Object.entries(record.evolucaoAnual || {}).map(([ano, data]) => ({
    ano: Number(ano),
    total: typeof data?.valor === 'number' ? data.valor : 0,
  }))

  const anos = accumulateYearlyData([anosFromArray, anosFromRecord, anosFromEvolucao])
  ...
  const numTransacoes = (() => {
    if (typeof record.numeroTransacoes === 'number') return record.numeroTransacoes
    if (typeof record.transacoes === 'number') return record.transacoes
    if (typeof record.numTransacoes === 'number') return record.numTransacoes
    if (typeof record.numero_transacoes === 'number') return record.numero_transacoes
    ...
    if (record.evolucaoAnual) {
      return Object.values(record.evolucaoAnual).reduce((sum: number, data) =>
...
export function normalizeDeputado(record: DeputyRecord): NormalizedDeputado {
  ...
  const anosFromArray = Array.isArray(record.anos) ? record.anos : []
  const anosFromRecord = Object.entries(record.gastosPorAno || {}).map(([ano, total]) => ({
    ano: Number(ano),
    total: typeof total === 'number' ? total : 0,
  }))

  const anos = accumulateYearlyData([anosFromArray, anosFromRecord])
...
// ============================================================================
// DADOS COMPLETOS DEPUTADO (Migrated from actions/analytics.ts)
// ============================================================================
